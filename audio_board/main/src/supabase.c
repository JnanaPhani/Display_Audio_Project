...
static void handle_device_status_record(cJSON *data_node) {
  ...
  // 4. Check volume adjustment
  cJSON *vol_item = cJSON_GetObjectItem(data_node, "volume");
  if (vol_item) {
    ...
    } else if (cJSON_IsNull(vol_item)) {
      nvs_handle_t handle;
      if (nvs_open("storage", NVS_READWRITE, &handle) == ESP_OK) {
        nvs_erase_key(handle, "device_volume");
        nvs_commit(handle);
        nvs_close(handle);
      }
    }
...
void send_device_health_to_supabase(void) {
  ...
  if (nvs_open("storage", NVS_READONLY, &handle) == ESP_OK) {
    ...
    nvs_get_i32(handle, "device_volume", &saved_volume);
    nvs_close(handle);
  }
  ...
  if (saved_volume >= 0) {
    cJSON_AddNumberToObject(root, "volume", saved_volume);
  } else {
    cJSON_AddNullToObject(root, "volume");
  }
...
void check_for_updates_via_http(void) {
  char url[256];
  snprintf(url, sizeof(url), "%s/rest/v1/system_control?device_type=eq.speaker", SUPABASE_URL);
