 ...
 *        scan/display path.
 *
 * Offline-first: app_main starts the barcode scanner + display immediately, and
 ...
 * device keeps scanning and displaying tokens.
 */
static void network_task(void *pvParameters)
{
    ...
    // ESP-NOW: bring up scan-broadcast to the gateway now that Wi-Fi is started.
...
void app_main(void)
{
    ...
    ESP_LOGW("DEBUG", "=============================================");
    ESP_LOGW("DEBUG", "SMART TOKEN DISPLAY (OTA & Realtime Enabled) (%s)", FIRMWARE_VERSION);
    ESP_LOGW("DEBUG", "=============================================");
    ...
    // Start the P10 display + USB-HID barcode scanner FIRST and unconditionally.
    // Scanning and displaying tokens must work even if Wi-Fi is unprovisioned or
    // disconnected. Scans that can't reach the DB are queued in NVS and flushed
    // once connectivity returns (see token_display.c).
    token_display_start();

    // --- Networking runs in the background; it must never block scanning. ---
...
void initialize_sntp(void)
{
    ...
    /* Set timezone to India Standard Time (UTC+5:30) so log timestamps and any
     * localtime_r()-based code (e.g. today_iso() in token_display.c) read IST
     * instead of UTC. POSIX TZ sign is inverted: UTC+5:30 -> "IST-5:30". */
    ...
    if (timeinfo.tm_year < (2023 - 1900)) {
        // Offline-first: do NOT reboot on time-sync failure. The device may be
        // offline or have no NTP reachability; rebooting would interrupt
        // scanning/display. SNTP keeps polling in the background and will set
        // the time once a network is available.
