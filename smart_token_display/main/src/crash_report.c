...
void crash_report_check_and_upload(void) {
  ...
  if (summary && esp_core_dump_get_summary(summary) == ESP_OK) {
    ...
    for (uint32_t i = 0; i < depth && off < sizeof(bt_str) - 12; i++) {
      off += snprintf(bt_str + off, sizeof(bt_str) - off, "0x%08" PRIx32 " ",
  ...
  cJSON_AddStringToObject(root, "p_reboot_reason",
                          G_REBOOT_REASON_STR ? G_REBOOT_REASON_STR
                                              : "Unknown");
  if (crashed_task[0])
    cJSON_AddStringToObject(root, "p_crashed_task", crashed_task);
  else
    cJSON_AddNullToObject(root, "p_crashed_task");
  if (pc_str[0])
    cJSON_AddStringToObject(root, "p_fault_pc", pc_str);
  else
    cJSON_AddNullToObject(root, "p_fault_pc");
  if (bt_str[0])
    cJSON_AddStringToObject(root, "p_backtrace", bt_str);
  else
    cJSON_AddNullToObject(root, "p_backtrace");
  cJSON_AddNullToObject(root, "p_uptime_before_crash");

  char *body = cJSON_PrintUnformatted(root);

  ESP_LOGW(TAG_CR, "reporting crash: task=%s pc=%s reason=%s",
           crashed_task[0] ? crashed_task : "?", pc_str[0] ? pc_str : "?",
